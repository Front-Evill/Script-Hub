...
local function sendWebhook()
    ...
    local requestFunc
    pcall(function()
        requestFunc = syn and syn.request or http and http.request or request or HttpPost
    end)
...
local AutoScriptFly = Tabs.Main:AddSection("سكربت طيران")
...
local FaemFofSE = Tabs.Setting:AddSection("ازاله الضباب")
...
FarmFpsQuSetting:AddButton({
    ...
    Callback = function(state)
        if state then
            Notify("Ez" , "now the game so coll" , 10)
           else
            Notify("Oops" , "The code is not working" , 10) 
        end
...
FarmMoodHub:AddButton({
 ...
 local ScriptAim = Tabs.Scain:AddSection("انيميشن")
