...
local function CreateHighlight(obj, color, isPlayer)
    if obj and ((isPlayer and obj:FindFirstChild("HumanoidRootPart")) or (not isPlayer and obj:IsA("BasePart"))) then
        ...
        highlight.FillTransparency = isPlayer and 0.4 or 0.2
...
local function UpdateCoinHighlights()
    ...
    for _, coin in pairs(coins) do
        if coin and coin.Parent then
            local highlight = CreateHighlight(coin, Color3.fromRGB(255, 215, 0), false)
            if highlight then
...
local function MoveToCoin(coin)
    ...
    if duration > 1.0 then duration = 1.0 end
    ...
    CurrentTween.Completed:Connect(function()
        if coin and coin.Parent and ValidateCharacter() and isRoundActive and not isDead then
            spawn(function()
                ...
                while originalCoin and originalCoin.Parent and attempts < 5 and ValidateCharacter() and not isDead do
                    attempts = attempts + 1
                    
                    humanoidRootPart.CFrame = CFrame.new(originalCoin.Position + Vector3.new(0, 0.3, 0))
        ...
        if otherPlayer ~= player and otherPlayer.Character and 
           ...
           otherPlayer.Character.Humanoid.Health > 0 then
            ...
            pcall(function()
                ...
                local targetCFrame = CFrame.new(otherRoot.Position + Vector3.new(0, 1.5, 0))
    ...
    pcall(function()
        for _, otherPlayer in pairs(Players:GetPlayers()) do
            if otherPlayer ~= player and otherPlayer.Character and otherPlayer.Character:FindFirstChild("HumanoidRootPart") then
                ...
                game:GetService("Debris"):AddItem(bodyVelocity, 1.0)
            end
        end
    end)
end
            ...
            pcall(function()
                ...
                if ValidateCharacter() and isRoundActive and not isDead then
                    ...
                    if closestCoin then
                        ...
                        wait(2.0) -- Changed to 2 seconds as requested
                    else
                        wait(0.5)
                    end
                else
