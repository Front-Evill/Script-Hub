...
palyerTab:AddSection({
    Name = "anemothshen"
})


-- تعريف المشيات المختلفة
local walkStyles = {
    ["مشية طبيعية"] = {
        animation = nil,
        walkSpeed = 16
    ...
    ["مشية زومبي"] = {
        animation = "http://www.roblox.com/asset/?id=616163682",
        walkSpeed = 12
    },
    ["مشية ملكية"] = {
        animation = "http://www.roblox.com/asset/?id=941028902",
        walkSpeed = 16
    },
    ["مشية نينجا"] = {
        animation = "http://www.roblox.com/asset/?id=656118852",
        walkSpeed = 20
    },
    ["مشية روبوت"] = {
        animation = "http://www.roblox.com/asset/?id=616091570",
        walkSpeed = 14
    },
    ["مشية رشيقة"] = {
        animation = "http://www.roblox.com/asset/?id=910034870",
        walkSpeed = 18
    },
    ["مشية شبح"] = {
        animation = "http://www.roblox.com/asset/?id=616006778",
        walkSpeed = 16
    }
}

-- متغير لتخزين المشية الحالية
local currentWalkStyle = "مشية طبيعية"
local animateScript = nil

-- دالة لتحديث المشية
local function updateWalkStyle(walkStyle)
    local player = game:GetService("Players").LocalPlayer
    if not player or not player.Character then return end
    
    -- تحديث سرعة المشي
    if player.Character:FindFirstChild("Humanoid") then
        player.Character.Humanoid.WalkSpeed = walkStyles[walkStyle].walkSpeed
    end
    
    -- العثور على سكربت الرسوم المتحركة
    if not animateScript then
        for _, child in pairs(player.Character:GetChildren()) do
            if child:IsA("Script") and child.Name == "Animate" then
                animateScript = child
                break
            end
        end
    ...
    -- تطبيق المشية الجديدة
    if animateScript and walkStyles[walkStyle].animation then
        -- تغيير جميع حالات المشي
        for _, animationState in pairs({"walk", "run"}) do
            if animateScript:FindFirstChild(animationState) then
                local numberValue = animateScript[animationState]:FindFirstChild("WalkAnim")
                if numberValue then
                    numberValue.AnimationId = walkStyles[walkStyle].animation
                end
            end
        end
    elseif animateScript and not walkStyles[walkStyle].animation then
        -- إعادة التعيين إلى المشية الافتراضية
        for _, animationState in pairs({"walk", "run"}) do
            if animateScript:FindFirstChild(animationState) then
                local numberValue = animateScript[animationState]:FindFirstChild("WalkAnim")
                if numberValue then
                    -- إعادة تعيين إلى الرسوم المتحركة الافتراضية
                    if animationState == "walk" then
                        numberValue.AnimationId = "http://www.roblox.com/asset/?id=180426354"
                    else
                        numberValue.AnimationId = "http://www.roblox.com/asset/?id=180426354"
                    end
                end
    ...
    -- تعيين المشية الحالية
    currentWalkStyle = walkStyle
end



 إضافة قائمة منسدلة للمشيات
playerTab:AddDropdown({
    Name = "اختر المشية",
    Options = {"مشية طبيعية", "مشية زومبي", "مشية ملكية", "مشية نينجا", "مشية روبوت", "مشية رشيقة", "مشية شبح"},
    Default = "مشية طبيعية",
    Flag = "selectedWalkStyle",
    Callback = function(Value)
        updateWalkStyle(Value)
        mainTab:AddParagraph({
            Title = "الحالة",
            Text = "تم تطبيق المشية: " .. Value
        })
    end
})
